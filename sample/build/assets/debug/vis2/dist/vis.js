...
Node.prototype._label = function (ctx, text, x, y, align, baseline) {
  if (text && this.fontSize * this.graphScale > this.fontDrawThreshold) {
    ctx.font = (this.selected ? "bold " : "") + this.fontSize*1.5 + "px " + this.fontFace;
    ...
    var lines = text.split('\n'),
        ...
        yLine = y + (1 - lineCount) / 2 * fontSize - 40;
...
Edge.prototype._label = function (ctx, text, x, y) {
  if (text) {
    ...
    ctx.font = ((this.from.selected || this.to.selected) ? "bold " : "") +
        this.fontSize*1.5 + "px " + this.fontFace;
    ctx.fillStyle = 'white';
...
var manipulationMixin = {
    ...
    if (this.editMode == true) {
      ...
      this.manipulationDiv.innerHTML = "" +
        ...
        "<span class='graph-manipulationUI connect' id='graph-manipulate-connectNode'>" +
          "<span class='graph-manipulationLabel'>Add Edge</span></span>";
        //+ "<span class='graph-manipulationUI triple' id='graph-manipulate-addTriple'>" +
        //   "<span class='graph-manipulationLabel'>Add Triple</span></span>";
      if (this._getSelectedNodeCount() == 1 && this.triggerFunctions.edit) {
        ...
        "<span class='graph-manipulationUI output' id='graph-manipulate-outputNode'>" +
            "<span class='graph-manipulationLabel'>Select Output</span></span>";  
      }
      if (this._getSelectedNodeCount() == 0 && this._getSelectedEdgeCount() == 1 && this.triggerFunctions.editEdge) {
        this.manipulationDiv.innerHTML += "" +
          // "<div class='graph-seperatorLine'></div>" +
          "<span class='graph-manipulationUI edit' id='graph-manipulate-edit-edge'>" +
            "<span class='graph-manipulationLabel'>Edit Edge</span></span>";
      }
      ...
      if (this._getSelectedNodeCount() == 1 && this.triggerFunctions.edit) {
        var outputButton = document.getElementById("graph-manipulate-outputNode");
        outputButton.onclick = this._selectOutputNode.bind(this);
      }

      if (this._getSelectedNodeCount() == 0 && this._getSelectedEdgeCount() == 1 && this.triggerFunctions.editEdge) {
        var editEdgeButton = document.getElementById("graph-manipulate-edit-edge");
        editEdgeButton.onclick = this._editEdge.bind(this);
      }
      if (this._selectionIsEmpty() == false) {
  ...
  _createAddNodeToolbar : function() {
    ...
    this.manipulationDiv.innerHTML = "" +
      ...
      "<span class='graph-manipulationUI none' id='graph-manipulate-back'>" +
        "<span class='graph-manipulationComment'>Click in an empty space to place a new node</span></span>";
  ...
  _createAddEdgeToolbar : function() {
    ...
    this.manipulationDiv.innerHTML = "" +
      ...
      // "<div class='graph-seperatorLine'></div>" +
      "<span class='graph-manipulationUI none' id='graph-manipulate-back'>" +
        "<span id='graph-manipulatorLabel' class='graph-manipulationComment'>Click on a node and drag the edge to another node to connect them.</span></span>";
  ...
  _finishConnect : function(pointer) {
    //加输入框
    //获取到label
    var edgeLabel = document.getElementById("edge-label-input").value;
      ...
      if (node != null) {
        if (node.clusterSize > 1) {
          ...
          if (edgeLabel.length > 0)
            this._createEdge(connectFromId,node.id, edgeLabel);
          else
   ...
   * @private
   */
  _createEdge : function(sourceNodeId,targetNodeId, linkLabel) {
    if (this.editMode == true) {
      var defaultData = {from:sourceNodeId, to:targetNodeId};
      if (linkLabel)
        defaultData.label = linkLabel;
  ...
  /**
   * Choose the selected node as output node
   *
   * @private
   */
  _selectOutputNode : function() {
      //console.log("hide");
      var node = this._getSelectedNode();
      var data;
      // console.log(node);
      // console.log(this.constants.nodes.color.background);
      if (node.color.background == this.constants.nodes.color.personalized) {
        return;
      }
      if (node.color.background == this.constants.nodes.color.output) {
        console.log("recover");
        data = {id:node.id,
        label: node.label,
        group: node.group,
        ...
        outputState: false,
        color: {
          background:this.constants.nodes.color.background,
          border:node.color.border,
          highlight: {
            background:node.color.highlight.background,
            border:node.color.highlight.border
        ...
        shape: node.shape,
        outputState: true,
        color: this.constants.nodes.color.output};
      }
      
        var me = this;
        
        me.nodesData.update(data);
        // me._createManipulatorBar();
        me.moving = true;
        me.start();
  ...
  /**
   * Edit edge: add label to the select edge
   *
   * @private
   */
  _editEdge : function() {
    if (this.triggerFunctions.editEdge && this.editMode == true) {
      var edge = this._getSelectedEdge();
      data = {
        id: edge.id,
        label: edge.label
      };
      if (this.triggerFunctions.editEdge.length == 2) {
        var me = this;
        this.triggerFunctions.editEdge(data, function (finalizedData) {
  ...
  _deleteSelected : function() {
    // cannot delete personalized node
    /*if (!this._selectionIsEmpty() && this.editMode == true) {
      if (!this._clusterInSelection()) {
        ...
        for (var curNodeId in selectedNodes) {
          var curNode = this.nodes[curNodeId];
          console.log(curNode);
          if (curNode.color.background == this.constants.nodes.color.personalized) {
            console.log("here");
            return;
          }
        }
...
function Graph (container, data, options) {
  ...
  this.triggerFunctions = {add:null,edit:null,connect:null,delete:null,editEdge:null,addEdge:null};
...
Graph.prototype.setOptions = function (options) {
  if (options) {
    ...
    if (options.onAddEdge) {
      this.triggerFunctions.addEdge = options.onAddEdge;
    }
